# Eye Disease Classification using ConvNeXtV2 

## Overview

This project develops a deep learning model to classify eye diseases from retinal images.  
The model is built using the **ConvNeXtV2 architecture** implemented in **PyTorch** to detect different types of eye diseases automatically.

Early detection of eye diseases is important to prevent vision loss and help medical professionals provide faster diagnosis.

This project was developed as part of an **undergraduate thesis for academic purposes**, titled:

**"Pengembangan Model Klasifikasi Citra Penyakit Mata Berbasis ConvNeXtV2"**

The research focuses on exploring the effectiveness of the ConvNeXtV2 architecture for medical image classification using retinal fundus images.

## Research Objectives

* Implement ConvNeXtV2 for medical image classification
* Train a model to detect eye diseases from retinal images
* Evaluate the model using Accuracy and F1 Score
* Analyze the performance of ConvNeXtV2 for eye disease detection

---

### Dataset

Ocular Disease Intelligent Recognition dataset containing retinal fundus images labeled with multiple eye diseases.

Categories include:

* Normal
* Bulging
* Crossed Eyes
* Cataract
* Hordeolum
* Uveitis


---


## Model Architecture

This project uses the **ConvNeXtV2** architecture, a modern convolutional neural network designed for high-performance image classification.

Framework used:

* Python
* PyTorch
* Deep Learning

---

## Results

Accuracy : **97.96%**
F1 Score : **0.92**

The results show that ConvNeXtV2 performs well in classifying retinal images and detecting eye diseases.

---

## Technologies

* Python
* PyTorch
* ConvNeXtV2
* Deep Learning

---

